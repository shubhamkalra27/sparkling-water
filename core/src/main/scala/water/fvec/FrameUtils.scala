...
trait FrameUtils {
  ...
  /**
    ...
    */
  def getChunksLocations(fr: Frame): Map[Int, NodeDesc] ={
    val cidxToH2ONode = (0 until fr.anyVec().nChunks()).map{ cidx =>
      (cidx, NodeDesc.fromH2ONode(fr.anyVec().chunkKey(cidx).home_node()))
    }.toMap
    cidxToH2ONode
  }
}
